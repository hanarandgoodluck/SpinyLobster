...
class TestCASE_0006(unittest.TestCase):
    ...
    def test_execute(self):
        ...
        print("[STEP] 验证当前URL为百度首页")
        # 验证当前URL为百度首页
        self.assertEqual(self.page.url, "https://www.baidu.com/", f"URL不匹配: 预期'https://www.baidu.com/', 实际'{self.page.url}'")

        print("[STEP] 验证百度搜索输入框存在且可见，表明页面核心功能已加载")
        # 验证百度搜索输入框存在且可见，表明页面核心功能已加载
        from playwright.sync_api import expect
        locator = self.page.locator("input[name="wd"]")
        try:
            expect(locator).to_be_visible(timeout=5000)
        except Exception:
            # 如果 to_be_visible 失败，尝试检查元素是否在DOM中
            self.assertTrue(locator.count() > 0, "元素不存在于DOM中: input[name="wd"]")
